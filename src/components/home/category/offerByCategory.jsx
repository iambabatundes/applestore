...
export default function OfferByCategory() {
  ...
  const fetchCategoriesAndProducts = async () => {
    try {
      ...
      if (allCategories.length > 0) {
        try {
          const featuredProductsResponse = await getProductsByCategorys(
            allCategories[0]._id
          );
          const products =
            featuredProductsResponse.data || featuredProductsResponse;
  ...
  const getCategoryImage = (imageData) => {
    ...
    if (typeof imageData === "string") {
      return imageData.startsWith("http")
        ? imageData
        : `${import.meta.env.VITE_API_URL}${imageData}`;
    }
    ...
    if (typeof imageData === "object") {
      if (imageData.url) {
        return imageData.url.startsWith("http")
          ? imageData.url
          : `${import.meta.env.VITE_API_URL}${imageData.url}`;
      }
  ...
  return (
    <div className="offers-container">
      <h2 className="offers-title">Offers by Category</h2>
                        ...
                        <div className="product-image">
                          <img
                            src={getCategoryImage(
                              product.featureImage || product.media?.[0]
                            )}
                            alt={product.name}
                            onError={(e) => {
