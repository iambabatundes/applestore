...
import "./styles/adminSetupScreens.css"; // Add the CSS we created
import { AdminService, ERROR_TYPES } from "../../../services/adminService";
import { useAdminAuthStore } from "../store/useAdminAuthStore";

// Your existing components (PasswordStrength, Toast, QRCodeDisplay, BackupCodes, ProgressSteps)
// ... (keeping all your existing component definitions)
...
// Main Enhanced Admin Setup Component with proper integration
const AdminSetup = ({ onSetupComplete, darkMode = false }) => {
  ...
  // Get admin auth functions for token management
  const { setAuthState } = useAdminAuthStore();
    ...
    try {
      switch (currentStep) {
        ...
        case 1: // Create initial admin
          const adminResult = await AdminService.createInitialAdmin({
        ...
        case 2: // Verify email
          const verifyResult = await AdminService.verifyEmail({
        ...
        case 3: // Complete 2FA setup
          const twoFAResult = await AdminService.complete2FA({
          ...
          if (twoFAData.accessToken) {
            setAuthState({
              adminUser: twoFAData.user || twoFAData.admin,
              accessToken: twoFAData.accessToken,
              refreshToken: twoFAData.refreshToken,
  ...
  const handleSetupCompletion = () => {
    console.log("[AdminSetup] Setup completed, calling onSetupComplete");
    if (onSetupComplete) {
      onSetupComplete();
    } else {
